...
int main(int argc, char* argv[]) {
    ...
    try {
        std::cout << "🚀 ContextEngine gRPC API Layer Starting..." << std::endl;
        std::cout << "Features: Streaming, Consistent Hashing, Backpressure" << std::endl;
        
        // Create and start gRPC server
        nscfstore::g_server = std::make_unique<nscfstore::GrpcContextEngineServer>("0.0.0.0:50051");
        
        if (!nscfstore::g_server->Start()) {
        ...
        nscfstore::TestStreamingIngestion();
        
        // Test 2: Relationship queries
        nscfstore::TestRelationshipQueries();
        
        // Test 3: Subgraph queries
        nscfstore::TestSubgraphQueries();
        
        // Test 4: Health checks
        nscfstore::TestHealthChecks();
        ...
        while (nscfstore::g_server->IsRunning()) {
            ...
            if (counter % 30 == 0) {
                ...
                if (nscfstore::g_client->HealthCheck("ContextEngine", true, health_resp)) {
    ...
    std::cout << "\n👋 ContextEngine shutting down gracefully..." << std::endl;
    return 0;
}
