...
void PartitionedBloomFilter::add(const std::string& item) {
    ...
    auto hash_keys = hash_keys(item);
    
    partitions_[partition_id].add(item, hash_keys[partition_id % hash_keys.size()]);
}
...
bool PartitionedBloomFilter::might_contain(const std::string& item) const {
    ...
    return partitions_[partition_id].might_contain(item, hash_keys[partition_id % hash_keys.size()]);
}
